...
### Binaries (`src/`)
- **`db-syncer`** — copies users, accounts, positions, and the username map from Redis to Postgres.
- **`trade-writer`** — reads booked trades off a Redis stream (written by the API) and writes them to Postgres.
- **`redis-populator`** — the reverse of `db-syncer`: reads users/accounts/positions/username from Postgres and rebuilds the Redis hashes. Runs once and exits — used for cold-cache bootstrap or restore after `make db-restore`/`make db-clear`.
