...
- **`manifests/overlays/dev-<name>/`** — one Kustomize overlay per developer, each able to override images/resources/replicas from base.
...
- **`compose.yml`** / **`Dockerfile.toolbox`** — the `k8s-toolbox` container: a sidecar with `kubectl`/`k3d`/`flux` that mounts the Docker/Podman socket, used by every `make` target to run cluster commands without installing tooling on the host.
- **`k3d-config.yaml`** — the local k3d cluster topology (nodes, port mappings).

### How it fits together
`cluster_up.sh` starts `k8s-toolbox`, creates the k3d cluster, applies `flux-system/` to bootstrap Flux, then applies the target for the selected developer (`./cluster_up.sh <name>`, default upstream). From there Flux reconciles `manifests/overlays/<target>/` into the cluster on its own; `make sync` forces an immediate reconciliation.
